class Solution:
    def uniquePathsWithObstacles(self, obstacleGrid):
        ...
        # Base case: the top-left cell is reachable if there's no obstacle there
        dp[0][0] = 1 if obstacleGrid[0][0] == 0 else 0

        # Fill in the first row: can only move right if there's no obstacle
        for j in range(1, n):
            dp[0][j] = dp[0][j - 1] if obstacleGrid[0][j] == 0 else 0

        # Fill in the first column: can only move down if there's no obstacle
        for i in range(1, m):
